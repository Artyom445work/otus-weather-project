...
import { getCurrentWeather, getCityCoords, WeatherData } from './openweather'
...
const createMap = (ll: string) => {
    const map = document.querySelector<HTMLImageElement>(".content-map-box__img")
...
const showWeather = (data: WeatherData) => {
    ...
    const weatherImage = document.querySelector<HTMLImageElement>(".content-weather-box__image")
...
router.addRoute({
    ...
    onEnter: async () => {
        ...
        document.querySelector(".interaction__button")?.addEventListener("click", (event: Event) => {
            event.preventDefault()
            const input = document.querySelector<HTMLInputElement>(".interaction__input")
            if (input?.value) {
        ...
        createMap(ll)
        const weather: WeatherData = await getCurrentWeather(cityName, openweathermapApiKey)
        showWeather(weather)
        renderHistory()
    },
})
...
router.addRoute({
    ...
    onEnter: async (uriCity: string) => {
        ...
        if (!interactionButton) {
            ...
            interactionButton?.addEventListener("click", (event: Event) => {
                event.preventDefault()
                const input = document.querySelector<HTMLInputElement>(".interaction__input")
                if (input?.value) {
                    router.navigate(`/city/${encodeURIComponent(input.value)}`)
